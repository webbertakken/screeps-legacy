...
RoomObject.prototype.log = function (message, severity = DEBUG, notify = false) {
  ...
  if (!this.room) {
    ...
    //room = this.name;
    room = this.toString(true);
  } else {
    //room = this.room.name;
    room = this.room.toString(true);
  }
...
/**
 * Return the room name and position as a string
 * @param html - return as html link
 * @returns {string}
 */
 ...
 * @returns {string}
 */
Room.prototype.toString = function (html = true) {
  if (html) {
